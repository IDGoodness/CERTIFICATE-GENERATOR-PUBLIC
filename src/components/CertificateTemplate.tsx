...
const CertificateTemplate: React.FC<CertificateTemplateProps> = ({
  ...
  const renderCustomTemplate = () => {
    // Normalize incoming config to support both older and newer shapes
    const config = customTemplateConfig || {};

    // If organizationLogo is set at the root of the config, map it into elements.logo.url for compatibility
    if (config.organizationLogo && !config.elements?.logo?.url) {
      config.elements = config.elements || {};
      config.elements.logo = config.elements.logo || {};
      config.elements.logo.url =
        config.elements.logo.url || config.organizationLogo;
    }

    // If signatureUrls were provided as a top-level array, map them into elements.signatures
    if (
      Array.isArray(config.signatureUrls) &&
      (!config.elements ||
        !config.elements.signatures ||
        config.elements.signatures.length === 0)
    ) {
      config.elements = config.elements || {};
      config.elements.signatures = (config.signatureUrls || []).map(
        (url: string, idx: number) => ({
          imageUrl: url,
          name: (config.signatureNames && config.signatureNames[idx]) || "",
          title: undefined,
        })
      );
    }
    ...
    const borderStyle: React.CSSProperties = {
      ...
      borderColor:
        config.colors?.border || config.colors?.accentColor || "#ea580c",
    };
  ...
  const getTemplateStyles = () => {
    ...
    const primaryColor =
      subsidiary?.settings?.primaryColor ||
      subsidiary?.primaryColor ||
      "#6366f1";
  ...
  const CertificateSeal = () => {
    ...
    return (
      <div className="absolute bottom-8 right-8 flex flex-col items-center">
        ...
        >
          <SealIcon className="w-10 h-10" style={{ color: styles.accent }} />
        </div>
        <div className="mt-2 text-center">
          ...
          <div className="text-xs text-gray-600 font-mono">{certificateId}</div>
        </div>
      </div>
    );
  };
        ...
        <div className="transform rotate-45 opacity-5">
          <div className="grid grid-cols-6 gap-8">
            {Array.from({ length: 24 }).map((_, i) => (
              <Award key={i} className="w-12 h-12 text-gray-400" />
            ))}
          </div>
        </div>
      </div>
  ...
  const CompletionDetails = () => {
    ...
    const hasSignatories =
      settings && settings.signatories && settings.signatories.length > 0;
          ...
          <>
            ...
            <div className="flex justify-between items-start gap-2">
              <div className="text-left flex-1 min-w-0">
                <p className="text-gray-600 mb-0.5 text-[10px]">
                  Date of Completion
                </p>
                <p
                ...
                {settings!.signatories!.slice(0, 2).map((signatory) => (
                  <div key={signatory.id} className="text-center">
                    ...
                    <div
                      ...
                      style={{ backgroundColor: styles.accent }}
                    />
                    <p className="text-[10px] font-semibold text-gray-800 truncate max-w-24">
                      {signatory.name}
                    </p>
                    <p className="text-[9px] text-gray-600 truncate max-w-24">
                      {signatory.title}
                    </p>
                  </div>
                ))}
